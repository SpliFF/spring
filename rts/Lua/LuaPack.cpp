...
{
	LuaPushNamedCFunc(L, "pack",   l_pack);
	LuaPushNamedCFunc(L, "unpack",  l_unpack);
	return true;
}
...
int LuaPack::l_unpack(lua_State *L) 		/** unpack(s,f,[init]) */
{
 size_t len;
...
int LuaPack::l_pack(lua_State *L) 		/** pack(f,...) */
{
 int i=2;
